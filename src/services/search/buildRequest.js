...
function buildMatch (searchTerm) {
  return searchTerm
    ? {
      multi_match: {
        query: searchTerm,
        fields: ['title', 'description', 'editors.name', 'songs.artist', 'songs.title', 'tags', 'user.name']
      }
    }
    : { match_all: {} }
}
...
export default function buildRequest (state) {
  ...
  const body = {
    ...
    _source: ['uuid', 'title', 'description', 'editors', 'songs', 'tags', 'user', 'createdAt'],
    // aggs: {
    //   title: { terms: { field: 'title.keyword' } },
    //   description: { terms: { field: 'description.keyword' } },
    //   tags: { terms: { field: 'tags.keyword' } },
    //   editors: { terms: { field: 'editors.name.keyword' } },
    //   artist: { terms: { field: 'songs.artist.keyword' } },
    //   songtitle: { terms: { field: 'songs.title.keyword' } }
    // },
